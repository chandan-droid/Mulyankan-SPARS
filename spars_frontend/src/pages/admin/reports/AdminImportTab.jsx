...
export default function AdminImportTab() {
  ...
  const handleFileUpload = async (e) => {
    ...
    setImporting(true);
    try {
      const rows = await parseMarksExcel(file);
      setImportRows(rows);
        ...
        <Card className="glass-card overflow-hidden animate-fade-in-up">
          <CardHeader className="bg-muted/20 border-b border-border/30 flex flex-row items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button size="sm" className="btn-gradient text-white rounded-xl gap-1.5 text-xs h-8" onClick={() => { toast.success('Data confirmed! (Connect to saveMark to persist)'); setImportRows([]); }}>
